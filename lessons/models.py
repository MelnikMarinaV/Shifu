...
class Lesson(models.Model):  # Модель урока
    ...
    )  # Описание урока-здесь теоретические материалы к уроку
...
class Task(models.Model):  # Задания к урокам
    ...
    task_description = models.TextField(
        default=""
    )  # Описание задания - будет хранить "ответ" на задания по аудированию

    def __str__(
        self,
    ) -> str:  # Для удобного представления задания - отображается своим именем
        return self.title
...
class TaskSubmission(models.Model):  # Ответы на задания
    ...
    comment = models.TextField(
        default="", null=True, blank=True
    )  # хранения текстового комментария к ответу (например, от преподавателя)
    result_file = models.CharField(
        max_length=200, null=True, blank=True
    )  # поле для хранения пути к файлу, который пользователь загрузил в качестве ответа
